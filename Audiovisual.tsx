...
import { audiovisualProjects } from "../data";
...
export function Audiovisual() {
  ...
  const allTools = Array.from(new Set(audiovisualProjects.flatMap(p => p.tools.map(t => t.name))));

  const filteredProjects = audiovisualProjects.filter(p => {
    ...
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-6rem)] w-full py-10 px-4">
      ...
      >
        ...
        <p className="text-white/50 max-w-lg mx-auto text-sm md:text-base">
          Exploración de piezas en movimiento, edición de video y motion graphics con un enfoque narrativo profundo.
        </p>
        ...
        {displayProjects.length > 0 && (
          ...
          >
            ...
            <Link 
              ...
              className="px-6 py-3 bg-[#E8B931] text-black font-semibold rounded-full hover:bg-white transition-colors flex-shrink-0"
            >
              Ver Proyecto
            </Link>
          </motion.div>
